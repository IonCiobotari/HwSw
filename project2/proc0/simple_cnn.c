...
int main(int argc, char **argv){
  ...
  *semaphore = P0_START;
  ...
  for (image_to_classify = IMAGE_TO_CLASSIFY;
    ...
    printf("%d\t%d\t%f\n", image_to_classify, prediction, matSoftM[prediction]*100);
    if(image_to_classify == IMAGE_TO_CLASSIFY+NUMBER_OF_IMAGES_TO_CLASSIFY-1){
    	break;
    }
    while(*semaphore != P1_END){};
    printf("Image %d: digit %d %f\n", image_to_classify+1, *result, *result_percent);
  ...
  printf("Execution time: %.2f us\n", (double)((tEnd -tStart)/(COUNTS_PER_SECOND/1000000)));
}
